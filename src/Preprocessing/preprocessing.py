...
import numpy as np
import pandas as pd
from sklearn.preprocessing import LabelEncoder, Normalizer, StandardScaler, MinMaxScaler, PolynomialFeatures,OneHotEncoder
...
class Preprocessing_Pipeline():
    ...
    def fit_transform(self, df: pd.DataFrame):
        ...
        # Nominal features so we used (one hot encoding) not label encoding (which is in case of ordinal features).
        self.one_hot_encoder_season = OneHotEncoder(sparse_output=False,handle_unknown="ignore")
        season_one_hot_encoded = self.one_hot_encoder_season.fit_transform(
            df[[fenum.SEASON.value]]
        )
        #df.drop(columns= [fenum.SEASON.value],inplace = True)
    ...
    def transform(self, df: pd.DataFrame, label_encoder_season, label_encoder_store):
        ...
        df[fenum.SEASON.value] = df[fenum.MONTH.value].apply(getseason)
        df[fenum.SEASON.value] = label_encoder_season.transform(df[fenum.SEASON.value]) # Only transform without fitting.
    ...
    def polynomial_feature(self, x, x_val=None): # Train & Val Set without target
